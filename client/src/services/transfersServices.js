...
export const getTransfers = async () => api.get("/transfers").then((res) => res.data.transfers); // { transfers: [...] }
...
// Temp Transfer
export const getTempTransfer = async () => api.get("/transfers/temp").then((res) => res.data.temp); // { tempTransfer: { fromLocation, toLocation, items } }
export const createTempTransfer = async (tempTransfer) => api.post("/transfers/temp/init", tempTransfer).then((res) => res.data.temp); // { tempTransfer: { fromLocation, toLocation, items } }
...
export const finalizeTempTransfer = async () => api.post("/transfers/temp/finalize").then((res) => res.data.transfer); // { transfer: { id, fromLocation, toLocation, items } }