...
const TempTransferCard = ({ tempTransfer, onFinalize }) => {
    logInfo('tempTransfer', tempTransfer)
    if (!tempTransfer || tempTransfer.items.length === 0) {
    ...
    return (
        <div className="p-4 border border-yellow-400 rounded shadow-md bg-yellow-50">
            <div className="mb-3" >
                <p ><strong>From:</strong>{tempTransfer.fromLocation.name || 'N/A'} </p>
                <p><strong>To:</strong>{tempTransfer.toLocation.name || 'N/A'}</p>
            </div>
            <ul className="mb-3">
                {tempTransfer.items.map((item) => (
